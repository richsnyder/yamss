...
server::create(const std::string& a_url)
{
  ...
  if (pos == std::string::npos)
  {
    ...
    ye.set_what(boost::str(fmt % a_url));
    throw ye;
  }
  ...
  {
    YamssException ye;
    ye.set_what("Could not create a directory within which to run a job");
    throw ye;
  }
...
server::finalize(const JobKey& a_job)
{
  ...
  if (job == m_jobs.end())
  {
    ...
    ye.set_what(boost::str(fmt % a_job));
    throw ye;
  }
...
server::runner_pointer
server::get_runner(const JobKey& a_job)
{
  typename jobs_type::const_iterator job = m_jobs.find(a_job);
  if (job == m_jobs.end())
  {
...
server::getInterface(const JobKey& a_job, const std::int64_t a_loadKey)
{
  ...
  patch.set_activeDofs(structure_->get_active_dofs());
  patch.resize_nodeCoordinates(n_dofs * n_nodes);
  patch.resize_elementTypes(n_elements);
  patch.clear_elementVertices();
  ...
  try
  {
    ...
    for (n = 0, np = beg; np != end; ++n, ++np)
    {
      ...
      for (size_type dof = 0; dof < 6; ++dof)
      {
        ...
        {
          patch.set_nodeCoordinates(offsets[dof] + n, position(n));
        }
      }
    }
  }
    ...
    for (n = 0, ep = beg; ep != end; ++n, ++ep)
    {
      ...
      switch (elem_.get_shape())
      {
        case element_type::POINT:
          patch.set_elementTypes(n, ElementType::POINT);
          break;
        case element_type::LINE:
          patch.set_elementTypes(n, ElementType::LINE);
          break;
        case element_type::TRIANGLE:
          patch.set_elementTypes(n, ElementType::TRIANGLE);
          break;
        case element_type::QUADRILATERAL:
          patch.set_elementTypes(n, ElementType::QUADRILATERAL);
          break;
      }
      for (size_type m = 0; m < vertices.size(); ++m)
      {
        patch.push_back_elementVertices(vertices[m]);
      }
    }
  }
  catch (std::runtime_error& e)
...
server::getInterfaceMovement(const JobKey& a_job, const int64_t a_loadKey)
{
  ...
  movement.resize_displacements(n_dofs * n_nodes);
  movement.resize_velocities(n_dofs * n_nodes);
  movement.resize_accelerations(n_dofs * n_nodes);
  ...
  try
  {
    ...
    for (n = 0, np = beg; np != end; ++n, ++np)
    {
      ...
      for (size_type dof = 0; dof < 6; ++dof)
      {
        ...
        {
          pos = offsets[dof] + n;
          movement.set_displacements(pos, node_.get_displacement(dof, q));
          movement.set_velocities(pos, node_.get_velocity(dof, dq));
          movement.set_accelerations(pos, node_.get_acceleration(dof, ddq));
        }
      }
    }
  }
...
server::getNode(const JobKey& a_job, const int64_t a_nodeKey)
{
  ...
  Node node;
  node.set_position(converter::from(x));
  node.set_displacement(converter::from(u));
  node.set_force(converter::from(f));
  return node;
}
...
server::getState(const JobKey& a_job)
{
  ...
  State state;
  state.set_step(eom_->get_step(0));
  state.set_time(eom_->get_time(0));
  state.set_displacement(converter::from(eom_->get_displacement(0)));
  state.set_velocity(converter::from(eom_->get_velocity(0)));
  state.set_acceleration(converter::from(eom_->get_acceleration(0)));
  state.set_force(converter::from(eom_->get_force(0)));
  return state;
}
  ...
  try
  {
    ...
    for (n = 0, np = beg; np != end; ++n, ++np)
    {
      ...
      for (size_type dof = 0; dof < 6; ++dof)
      {
        if (structure_->is_active(dof))
        {
          f[dof] = a_loading.get_forces()[offsets[dof] + n];
        }
        else
        {
          f[dof] = 0.0;
